...
class RobImp(override val wrapper: Rob)(implicit p: Parameters, params: BackendParams) extends LazyModuleImp(wrapper)
  ...
  io.enq.hasLoadStore := hasLoadStore || VecInit(io.enq.req.map(e => e.valid && CommitType.isLoadStore(e.bits.commitType))).asUInt.orR
  io.enq.hasTileLS    := hasTileLS    || VecInit(io.enq.req.map(e => e.valid && CommitType.isTileLS(e.bits.commitType))).asUInt.orR

...
trait HasDCacheParameters extends HasL1CacheParameters with HasL1PrefetchSourceParameter with TmuParams {
  ...
  def reqIdWidth = (log2Up(nEntries) max log2Up(StoreBufferSize) max log2Up(tileLSQ_sz)) + 1 // 1 bit to distinguish between sbuffer and tmu
